...
app.post("/createRoom",(req,res)=>{
    try{
        ...
        var input_data={seats:req.body.seats,price:req.body.price,amenities:req.body.amenities,id:id_room,bookings:[]};
...
app.post("/bookRoom",function (req,res){
    ...
    var time_front_data=[req.body.start,req.body.end],dob_front=req.body.date; // Time and date of booking as provided by user
   try{
       if(rooms.length==0){
        ...
        rooms.forEach((room)=>{
            ...
            if(room.seats==req.body.seats_req){
                ...
                if(room.bookings.length==0){
                    var temp_variable={cust_id:id_customer,start:req.body.start,end:req.body.end,date:req.body.date};  //Update booking info of room
                    room.bookings.push(temp_variable);
                    var customer={name:req.body.name,customer_id:id_customer,date_of_booking:req.body.date,start_time:time_front_data[0],end_time:time_front_data[1],room_id:room.id};   //Creating Customer entry
                    ...
                    room.bookings.forEach((t)=>{
                        let time_back_data=[t.start,t.end] ,dob_back =t.date; // Time and date of booking already done
                ...
                if(flag==rooms.length)
                    throw "no room with seats required available"
...
function clash(t_back,d_back,t_front,d_front){
    ...
    if(d_back==d_front){
        if(t_back[0]==t_front[0]){
            flag=1;
         }else if((t_front[0]>=t_back[0] && t_front[0]<t_back[1]) || (t_back[0]>=t_front[0] && t_back[0]<= t_front[1])){
            flag=1;
        }
    }else{
        flag=0;
